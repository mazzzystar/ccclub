...
async function isGrokSubagent(file: string): Promise<boolean> {
  const summary = asRecord(await readJsonFile(join(dirname(file), "summary.json")));
  return asString(summary?.session_kind)?.toLowerCase().startsWith("subagent") ?? false;
}
...
function usageRows(
  ...
  return rows.length > 0 ? rows : [[defaultModel, usage]];
}
...
async function scanGrokFile(file: string): Promise<GrokScanResult> {
  ...
  const summary = asRecord(await readJsonFile(join(dirname(file), "summary.json")));
  const defaultModel = asString(summary?.current_model_id) ?? "unknown";
  await readJsonlFile(file, (value) => {
    ...
    const timestamp = toIsoTimestamp(meta?.agentTimestampMs ?? record?.timestamp);
    ...
    for (const [model, modelUsage] of usageRows(usage, defaultModel)) {
...
export async function collectGrokUsage(context: CollectorContext): Promise<SourceCollection> {
  ...
  for (const file of files) {
    // summary.json can appear after updates.jsonl was cached, so this check must stay outside the cache.
    if (await isGrokSubagent(file)) continue;

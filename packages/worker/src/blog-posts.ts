...
export const BLOG_POSTS: BlogPost[] = [
  {
    ...
    body: `
      ...
      <p>Since the initial release, ccclub has grown to support multiple coding agents. It now reads usage logs from Claude Code, Codex, OpenCode, Amp, Grok, and Pi, and can also track Cursor once you run <code>ccclub sources enable cursor</code> (Cursor keeps no local usage logs, so that one is opt-in). If you use more than one, all of them show up in the same leaderboard.</p>

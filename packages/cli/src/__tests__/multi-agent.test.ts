...
describe("multi-agent collection", () => {
  ...
  it("falls back to top-level Grok usage and the agent timestamp", async () => {
    ...
    expect(result.entries[0].timestamp).toBe("2026-04-18T10:21:00.000Z");
  });
  ...
  it("skips Grok subagent ledgers already folded into their coordinator", async () => {
    const grokHome = await makeTempDir();
    const coordinator = join(grokHome, "sessions", "project", "coordinator");
    const subagent = join(grokHome, "sessions", "project", "subagent");
    await Promise.all([mkdir(coordinator, { recursive: true }), mkdir(subagent, { recursive: true })]);
    const completed = (eventId: string, inputTokens: number) => JSON.stringify({
      timestamp: 1_776_507_660,
      params: {
        sessionId: eventId,
        update: {
          sessionUpdate: "turn_completed",
          usage: { modelUsage: { "grok-4.6-build": { inputTokens, outputTokens: 10 } } },
        },
        _meta: { eventId },
      },
    });
    await Promise.all([
      writeFile(join(coordinator, "updates.jsonl"), completed("coordinator-event", 200)),
      writeFile(join(coordinator, "summary.json"), JSON.stringify({ session_kind: "coordinator" })),
      writeFile(join(subagent, "updates.jsonl"), completed("subagent-event", 100)),
      writeFile(join(subagent, "summary.json"), JSON.stringify({ session_kind: "subagent" })),
    ]);
    vi.stubEnv("GROK_HOME", grokHome);
    ...
    expect(result.entries).toHaveLength(1);
    expect(result.entries[0].requestId).toBe("coordinator-event:grok-4.6-build");
  });

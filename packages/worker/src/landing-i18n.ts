...
export const LANDING_T: Record<LandingLang, LandingT> = {
  en: {
    ...
    description:
      "Claude Code and Codex leaderboard among friends. Track coding agent token usage, costs, and active status across Claude Code, Codex, OpenCode, Amp, Grok, and pi-agent.",
    ogDescription:
      "Track Claude Code, Codex, OpenCode, Amp, Grok, and pi-agent token usage, costs, and active status with friends.",
    eyebrow: "Among friends",
    h1: "Claude Code & Codex leaderboard among friends.",
    tagline:
      "Track coding agent token usage, cost, active status, and agent mix across Claude Code, Codex, OpenCode, Amp, Grok, and pi-agent.",
    ...
    step3p:
      'Claude Code syncs at session end, and background sync picks up Codex, OpenCode, Amp, Grok, and pi-agent. Run <code class="mono">ccclub</code> or open the web dashboard.',
    howDetail1:
      "ccclub reads token counts, cost estimates, model names, and number of calls from local coding agent logs for Claude Code, Codex, OpenCode, Amp, Grok, and pi-agent. No prompts, responses, code, file paths, or conversation data ever leave your machine.",
  ...
  zh: {
    ...
    description:
      "和朋友一起的 Claude Code / Codex 排行榜。追踪 Claude Code、Codex、OpenCode、Amp、Grok、pi-agent 的 token 用量、费用和活跃状态。",
    ogDescription: "和朋友一起追踪 Claude Code、Codex、OpenCode、Amp、Grok、pi-agent 的 token 用量、费用和活跃状态。",
    eyebrow: "和朋友一起",
    h1: "和朋友一起的 Claude Code & Codex 排行榜。",
    tagline: "追踪 Claude Code、Codex、OpenCode、Amp、Grok、pi-agent 的 token 用量、费用、活跃状态和 agent 构成。",
    ...
    step3h: "查看排行榜",
    step3p:
      'Claude Code 在会话结束时自动同步，后台同步覆盖 Codex、OpenCode、Amp、Grok 和 pi-agent。运行 <code class="mono">ccclub</code> 或打开网页仪表盘。',
    howDetail1:
      "ccclub 只从本机的 coding agent 日志（Claude Code、Codex、OpenCode、Amp、Grok、pi-agent）读取 token 数、费用估算、模型名和调用次数。提示词、回复、代码、文件路径和对话内容永远不会离开你的电脑。",
  ...
  ja: {
    ...
    description:
      "友達と競う Claude Code / Codex リーダーボード。Claude Code、Codex、OpenCode、Amp、Grok、pi-agent のトークン使用量・コスト・アクティブ状況をトラッキング。",
    ogDescription:
      "Claude Code、Codex、OpenCode、Amp、Grok、pi-agent のトークン使用量・コスト・アクティブ状況を友達と一緒にトラッキング。",
    eyebrow: "友達と一緒に",
    h1: "友達と競う Claude Code & Codex リーダーボード。",
    tagline:
      "Claude Code、Codex、OpenCode、Amp、Grok、pi-agent のトークン使用量、コスト、アクティブ状況、エージェント構成をトラッキング。",
    ...
    step3h: "リーダーボードを見る",
    step3p:
      'Claude Code はセッション終了時に自動同期し、Codex・OpenCode・Amp・Grok・pi-agent はバックグラウンドで同期されます。<code class="mono">ccclub</code> を実行するか、Web ダッシュボードを開いてください。',
    howDetail1:
      "ccclub が読み取るのは、ローカルのエージェントログ（Claude Code、Codex、OpenCode、Amp、Grok、pi-agent）にあるトークン数・コスト見積もり・モデル名・呼び出し回数だけです。プロンプト、応答、コード、ファイルパス、会話内容がマシンの外に出ることはありません。",
  ...
  de: {
    ...
    description:
      "Claude Code & Codex Leaderboard unter Freunden. Verfolge Token-Verbrauch, Kosten und Aktivität über Claude Code, Codex, OpenCode, Amp, Grok und pi-agent.",
    ogDescription:
      "Token-Verbrauch, Kosten und Aktivität von Claude Code, Codex, OpenCode, Amp, Grok und pi-agent gemeinsam mit Freunden verfolgen.",
    eyebrow: "Unter Freunden",
    h1: "Das Claude Code & Codex Leaderboard unter Freunden.",
    tagline:
      "Token-Verbrauch, Kosten, Aktivität und Agent-Mix über Claude Code, Codex, OpenCode, Amp, Grok und pi-agent hinweg.",
    ...
    step3p:
      'Claude Code synchronisiert am Sessionende; Codex, OpenCode, Amp, Grok und pi-agent per Hintergrund-Sync. Führe <code class="mono">ccclub</code> aus oder öffne das Web-Dashboard.',
    howDetail1:
      "ccclub liest aus den lokalen Agent-Logs (Claude Code, Codex, OpenCode, Amp, Grok, pi-agent) nur Token-Zahlen, Kostenschätzungen, Modellnamen und Aufrufzahlen. Prompts, Antworten, Code, Dateipfade oder Gesprächsinhalte verlassen deinen Rechner nie.",
  ...
  ru: {
    ...
    description:
      "Рейтинг Claude Code и Codex среди друзей. Отслеживайте расход токенов, затраты и активность в Claude Code, Codex, OpenCode, Amp, Grok и pi-agent.",
    ogDescription:
      "Отслеживайте расход токенов, затраты и активность Claude Code, Codex, OpenCode, Amp, Grok и pi-agent вместе с друзьями.",
    eyebrow: "Среди друзей",
    h1: "Рейтинг Claude Code и Codex среди друзей.",
    tagline:
      "Расход токенов, затраты, активность и состав агентов: Claude Code, Codex, OpenCode, Amp, Grok и pi-agent.",
    ...
    step3p:
      'Claude Code синхронизируется в конце сессии; Codex, OpenCode, Amp, Grok и pi-agent — в фоне. Запустите <code class="mono">ccclub</code> или откройте веб-дашборд.',
    howDetail1:
      "ccclub читает из локальных логов агентов (Claude Code, Codex, OpenCode, Amp, Grok, pi-agent) только количество токенов, оценку затрат, названия моделей и число вызовов. Промпты, ответы, код, пути к файлам и содержимое диалогов никогда не покидают ваш компьютер.",
